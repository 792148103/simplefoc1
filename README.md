...
当前 CAN0 使用标准帧、500 kbit/s。节点默认 ID 为 `1`，接收控制帧 ID 为 `0x001`；现有 XML 已将接收邮箱 1 至 31 分别配置为 ID `0x001` 至 `0x01F`，后续多节点只需为每块板设置不同的 `kCanNodeId`。

控制帧采用小端 IEEE-754 `float32`：`byte0` 为命令，`byte1..4` 为浮点参数。未使用的字节填 `0`。

| 命令 | 字节 | 说明 |
|---|---|---|
| `0x01` | `b1=模式`，`b2=力矩类型` | 模式：`0` 位置、`1` 速度；力矩类型：`0` 电压、`1` FOC 电流 |
| `0x02` | `b1..b4=float32` | 目标位置 rad 或目标速度 rad/s，取决于当前模式 |
| `0x03` | `b1..b4=float32` | 轨迹最大速度 rad/s |
| `0x04` | `b1..b4=float32` | 加速度 rad/s2 |
| `0x05` | `b1..b4=float32` | 减速度 rad/s2 |
| `0x06` | 无参数 | 停止轨迹：位置模式保持当前位置，速度模式减为 0 |
| `0x07` | 无参数 | 请求状态帧 |

位置模式下，固件依据最大速度、加速度和减速度生成梯形位置轨迹，再将轨迹位置送入现有位置闭环。速度模式下，固件以设置的加减速度平滑改变现有速度环目标。CAN 不修改 PID 参数。

状态帧每 20 ms 交替发送，使用邮箱 0：

| CAN ID | 数据 `b0..b3` | 数据 `b4..b7` |
|---|---|---|
| `0x181` | 实际轴角度 `float32` rad | 实际轴速度 `float32` rad/s |
| `0x1A1` | `Iq` `float32` A | 故障字 `uint32` |
| `0x1C1` | `b0=命令`，`b1=结果`，`b2=模式`，`b3=力矩类型` | 故障字 `uint32` |

`0x1C1` 中结果为 `1` 表示命令已在主循环应用，`0` 表示参数非法、P000 已失能或故障锁存。CAN 中断不会直接调用 `motor2`，因此不会破坏 FOC 内环实时性。
