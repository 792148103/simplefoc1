...
void FOCMotor::loopFOC() {
  ...
  if (sensor) {
    ...
    if (!sensor->isValid()) {
      // 传感器失效时禁止继续输出上一拍的电角度。
      disable();
      SIMPLEFOC_MOTOR_ERROR("Sensor read failed");
      return;
    }
  }
...
int  FOCMotor::initFOC() {
  ...
  if(sensor){
    ...
    sensor->update();
    if (!sensor->isValid()) {
      SIMPLEFOC_MOTOR_ERROR("Sensor read failed after alignment");
      exit_flag = 0;
    } else {
      shaft_angle = shaftAngle();
    }
...
int FOCMotor::alignSensor() {
  ...
  // 对齐期间任何一次传感器读取失败都必须立即撤销相电压。
  // 否则 I2C 超时后转子会持续承受固定对齐磁场并快速发热。
  auto update_sensor = [this]() -> bool {
    sensor->update();
    if (!sensor->isValid()) {
      setPhaseVoltage(0, 0, 0);
      SIMPLEFOC_MOTOR_ERROR("Sensor read failed during alignment");
      return false;
    }
    return true;
  };
  ...
  if(sensor_direction == Direction::UNKNOWN){
    ...
    // 记录方向识别过程中的三个机械角度，用于排查“转子已转动但编码器未识别”的问题。
    if (!update_sensor()) return 0;
    ...
    SIMPLEFOC_MOTOR_DEBUG("Align start: ", start_angle);
    SIMPLEFOC_MOTOR_DEBUG("Align mid: ", mid_angle);
    SIMPLEFOC_MOTOR_DEBUG("Align end: ", end_angle);
    SIMPLEFOC_MOTOR_DEBUG("Align moved: ", moved);
