...
extern volatile uint32_t timeout_ms;
...
//  }
    ...
    err = R_IIC_MASTER_SlaveAddressSet(&g_i2c_master0_ctrl, 0x3C, I2C_MASTER_ADDR_MODE_7BIT);
...
//  }
    ...
    for(i = 0; i < oled_y; i++)
    {
        ...
        refresh_err = R_IIC_MASTER_SlaveAddressSet(&g_i2c_master0_ctrl, 0x3C, I2C_MASTER_ADDR_MODE_7BIT);
        assert(FSP_SUCCESS == refresh_err);

        // 发送整帧显示数据（129字节）
        refresh_err = R_IIC_MASTER_Write(&g_i2c_master0_ctrl, buf, sizeof(buf), true);
        assert(FSP_SUCCESS == refresh_err);
