...
float MagneticSensorI2C::getSensorAngle(){
  ...
  const int raw_count = getRawCount();
  if (raw_count < 0) return -1.0f;
...
int MagneticSensorI2C::getRawCount() {
  ...
  if (0U != currWireError) return -1;
  // read the data msb and lsb
	if (2U != wire->requestFrom(_conf.chip_address, (uint8_t)2)) {
    currWireError = 4U;
    return -1;
  }
	for (byte i=0; i < 2; i++) {
